 ...
 * @author Clovis
 */
    class PesqRelatMot extends JInternalFrame implements ActionListener, ItemListener {
  	/**
	 * 
	 */
	private static final long serialVersionUID = -4160327735033525077L;
        ...
        static final int xOffset = 18, yOffset = 23;
        
   /** Creates a new instance of Pesquisa */
    public PesqRelatMot(String titulo, ControlMoto controlMoto) {
...
//    		fis.close();
//    	}catch (IOException e) {
//			JOptionPane.showMessageDialog(null,"Erro ao ler informaÃ§Ãµes do sistema","ControlMoto",JOptionPane.INFORMATION_MESSAGE);
//		}
        ...
        jcbNomeMotorista = new JComboBox();jcbNomeMotorista.setEnabled(false);
        jcbNomeMotorista.addActionListener(this);//jcbNomeMotorista.addItemListener(this);
	...
	public void itemStateChanged(ItemEvent e) {
		...
		if (e.getSource() == jXDPDataFinal ){
			...
			if(((dtTerm - dtIni)/mspd) > 30 ){
				JOptionPane.showMessageDialog(this, "Período inválido", "ControlMoto", JOptionPane.CANCEL_OPTION);
...
//    			JOptionPane.showMessageDialog(this, "Opcao valida so para ordem de servico fechadas", "ControlMoto", JOptionPane.CANCEL_OPTION);
    ...
    public void actionPerformed(ActionEvent e){
    	if(e.getSource() == jcbNomeMotorista && !jcbNomeMotorista.getSelectedItem().toString().isEmpty()){
			if(status != "F" && status != null){
				JOptionPane.showMessageDialog(this, "Opção válida somente para ordem\nde servico fechadas", "ControlMoto", JOptionPane.CANCEL_OPTION);
    	...
    	if(e.getSource() == jrbAberta){
    		...
    		if(!jcbNomeMotorista.getSelectedItem().toString().isEmpty()){
    			JOptionPane.showMessageDialog(this, "Opção válida só para ordem\nde servico fechadas", "ControlMoto", JOptionPane.CANCEL_OPTION);
    			jcbNomeMotorista.setSelectedItem("");jcbNomeMotorista.setEnabled(false);jrbAberta.setSelected(false);jrbFechada.setSelected(false);
    			jrbCancelada.setSelected(false);
    		}
    		jcbNomeMotorista.setEnabled(false);jrbFechada.setSelected(false);jrbCancelada.setSelected(false);
    	}
    	if(e.getSource() == jrbFechada){
    		status = "F";
    		jcbNomeMotorista.setEnabled(true);
    	}
    	if(e.getSource() == jrbCancelada){
    		status = "C";
    		if(!jcbNomeMotorista.getSelectedItem().toString().isEmpty()){
    			JOptionPane.showMessageDialog(this, "Opção válida sá para ordem\nde serviço fechadas", "ControlMoto", JOptionPane.CANCEL_OPTION);
    			jcbNomeMotorista.setSelectedItem("");jcbNomeMotorista.setEnabled(false);jrbAberta.setSelected(false);jrbFechada.setSelected(false);
    			jrbCancelada.setSelected(false);
    		}
    		jcbNomeMotorista.setEnabled(false);jrbFechada.setSelected(false);jrbCancelada.setSelected(false);
    		...
    		if(((dtTerm - dtIni)/mspd) <= 30 && status != null){ //|| jrbAberta.isSelected() == true && jrbFechada.isSelected() == true && jrbCancelada.isSelected() == true ){// bgOrdemServico.isSelected()){
	        	if(dataInicio.equals(dataInicio) && dataFinal.equals(dataFinal) && jcbNomeMotorista.getSelectedItem().toString().isEmpty() && jrbFechada.isSelected() == true){
	        		try{
	        			...
	        			jasperViewer.setTitle("Control Moto - Relatório Total Motorista por Período");
...
//	        			jasperViewer.setModalExclusionType(ModalExclusionType.NO_EXCLUDE);
	        	...
	        	if(dataInicio.equals(dataInicio) && dataFinal.equals(dataFinal) && !jcbNomeMotorista.getSelectedItem().toString().isEmpty() && jrbFechada.isSelected() == true ){
	           		try{
	        			...
	        			JasperViewer jasperViewer = new JasperViewer(relat, false);
	        			jasperViewer.setTitle("Control Moto - Relatório Total Motorista por Período e Nome");
	        			jasperViewer.setZoomRatio(0.75f);
	        			jasperViewer.setBounds(100,110,750,600);
	        			//ImageIcon icone = new ImageIcon(".."+separadorPath+separadorPath+"ControlMoto"+separadorPath+"imagens"+separadorPath+"logo_cs_b.gif");
	        			ImageIcon icone = new ImageIcon(caminho+separadorPath+lerArquivoVO.getLogoTipo()+separadorPath+"logo_cs_b.gif");
	        	...
	        	JOptionPane.showMessageDialog(this, "Período inválido ou opção não selecionada", "ControlMoto", JOptionPane.CANCEL_OPTION);
				jXDPDataFinal.requestFocus();
    		}
		}
